...
CSV_SCHEMA = {
    ...
    'device_type':          'Detected compute device class: "nvidia_gpu", "apple_silicon", or "cpu_only"',
    'gpu_telemetry_available': 'True when explicit NVIDIA GPU telemetry was captured for this run',
    'gpu_avg_power_w':      'Average GPU power during inference (watts), when GPU telemetry is available',
    'gpu_energy_mwh':       'Total GPU energy during inference (milliwatt-hours), when GPU telemetry is available',
    'baseline_gpu_power_w': 'Idle GPU power measured before inference (watts), when GPU telemetry is available',
    'gpu_inference_avg_power_w': 'Baseline-subtracted GPU inference power (watts), when GPU telemetry is available',
    'gpu_inference_energy_mwh': 'Baseline-subtracted GPU inference energy (milliwatt-hours), when GPU telemetry is available',
...
def build_record(result, power, config):
    ...
    return {
        ...
        'device_type': power.get('device_type'),
        'gpu_telemetry_available': power.get('gpu_telemetry_available', False),
        'gpu_avg_power_w': power.get('gpu_avg_power_w'),
        'gpu_energy_mwh': power.get('gpu_energy_mwh'),
        'baseline_gpu_power_w': power.get('baseline_gpu_power_w'),
        'gpu_inference_avg_power_w': power.get('gpu_inference_avg_power_w'),
        'gpu_inference_energy_mwh': power.get('gpu_inference_energy_mwh'),
